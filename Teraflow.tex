...
\\TeraFlow è stato finanziato dall'unione europea per il programma di ricerca e innovazione 5G PPP \cite{5GPPP}, ed ha preso il nome di Horizon 2020 \cite{Horizon}. 
...
diverse funzionalità che garantiscono al controller dinamicità, autoriparazione, integrità e bilanciamento del carico \cite{D53} \cite{D14}.
...
collegamenti e servizi, in un database No-SQL\cite{D31}. La componente ottimizza l'accesso concorrente e garantisce la coerenza dei dati gestiti dai vari componenti del controller SDN.
...
\section{Componenti}
Le componenti TeraFlow sono classificate in due categorie; le componenti principali del sistema operativo e le netapp sovrapposte. 
...
Un utente di RPC non può distinguere dove viene eseguita la procedura chiamata, infatti viente chiamata come una procedura locale \cite{grpcArt2}.
...
il formato da utilizzare per i messaggi. Essa viene definita in un file proto, che a prima vista può sembrare un file di testo ordinario, ma in realtà i metodi con i parametri di input e i valori di ritorno.
Tutti i metodi specificati nella definizione di questa interfaccia possono essere invocati dal client da remoto.
...
\\Quando il client gRPC invoca un servizio gRPC, il lato client utilizza i protocol buffer per serializzare la chiamata di procedura remota nel formato appropriato. 
