\documentclass[12pt]{report}
\usepackage[top=2.5cm, bottom=2.5cm, left=4cm, right=2.5cm, head=21.75pt, centering]{geometry}
\linespread{1.5}
% Librerie utili
\usepackage[italian]{babel} % applicazione regole di scrittura per la lingua italiana 
\usepackage[utf8]{inputenc} % codifica UTF-8
\usepackage{scrlayer-scrpage} % stili pagina per il frontespizio
\ifoot[]{}
\cfoot[]{}
\usepackage{hyperref}
\ofoot[\pagemark]{\pagemark}
\pagestyle{scrplain}
\usepackage{mathptmx} % font Times New Roman (sim
\usepackage{graphicx}
\usepackage{csquotes} % per le citazioni "in blocco"
\appto{\bibsetup}{\raggedright}
\usepackage{titlesec} % per la formattazione dei titoli delle sezioni, capitoli etc.
\usepackage{float} % per il posizionamento delle immagini
\usepackage{listings} % per il codice di programmazione
\renewcommand{\lstlistingname}{Code}% Listing -> Codice
\usepackage{xcolor}  % stile del codice
\definecolor{mygreen}{rgb}{0,0.6,0}
\definecolor{mygray}{rgb}{0.5,0.5,0.5}
\definecolor{mymauve}{rgb}{0.58,0,0.82}
\definecolor{darkgray}{rgb}{.4,.4,.4}
\definecolor{navy}{HTML}{000080}
\definecolor{purple}{rgb}{0.65, 0.12, 0.82}
\definecolor{codepurple}{rgb}{0.58,0,0.82}
\definecolor{backcolour}{rgb}{0.95,0.95,0.92}
\titleformat{\chapter}[block]
  {\normalfont\LARGE\bfseries}{\thechapter.}{0.5em}{\LARGE}
\titlespacing*{\chapter}{0pt}{-20pt}{25pt}

\lstdefinelanguage{CLI}{
  keywords={feature, install,list},
  keywordstyle=\color{codepurple}\bfseries,
  identifierstyle=\color{black},
  sensitive=false,
}

\lstdefinelanguage{Python}{
    morekeywords={def, return, if, elif, else, for, while, break, continue, try, except, finally, class, from, import, as, pass, lambda, with, assert, yield, raise, True, False, None},
    sensitive=true,
    morecomment=[l]{\#},   % Line comment style
    morestring=[b]',       % Single quote string
    morestring=[b]",       % Double quote string
    keywordstyle=\color{blue}\bfseries,  % Style for keywords
    commentstyle=\color{green},          % Style for comments
    stringstyle=\color{red},             % Style for strings
    basicstyle=\ttfamily\small,          % Style for basic text
    backgroundcolor=\color{lightgray!10}, % Background color for the code
    showstringspaces=false,              % Don't show space in strings
}

% Define JSON language settings
\lstdefinelanguage{json}{
    basicstyle=\ttfamily\small,          % Style for basic text
    backgroundcolor=\color{lightgray!10},% Background color for the code
    showstringspaces=false,              % Don't show space in strings
    breaklines=true,                     % Break lines automatically
    literate=
      *{:}{{{\color{red}:}}}{1}
      {,}{{{\color{red},}}}{1},
}

\lstset{
   language=CLI,
   breaklines=true,
   extendedchars=true,
   basicstyle=\footnotesize\ttfamily,
   showstringspaces=false,
   showspaces=false,
   rulecolor=\color{black},
   numbersep=9pt,
   tabsize=2,
   breaklines=true,
   showtabs=false,
   captionpos=b,
   backgroundcolor=\color{backcolour},
   frame=single,
}
\setlength{\parindent}{20pt}

\begin{document}

\begin{titlepage}
\begin{figure}
    \centering\includegraphics[scale=0.5]{cherubino_pant541.png}
\end{figure}

\begin{center}
    {\LARGE{ Corso di Laurea in Informatica \\}}
    \vspace{2cm}
    {\Large { TESI DI LAUREA }}\\
    \vspace{2cm}
    {\Large { Studio della gestione degli intenti nel controller SDN Teraflow }}
\end{center}

\vspace{2cm}

\begin{minipage}[t]{0.47\textwidth}
	{\large{\bf Relatore:\\ Federica Paganelli}}
	\vspace{0.5cm}
	{\large{\bf \\Controrelatore:\\ Stefano Forti}}
\end{minipage}\hfill
\begin{minipage}[t]{0.47\textwidth}\raggedleft
	{\large{\bf Candidato: \\ Carolina Ferrari\\ }}
\end{minipage}

\vspace{25mm}

\centering{\bf ANNO ACCADEMICO 2023/2024 }
\end{titlepage}

%\cleardoublepage
%\thispagestyle{empty}
%\vspace*{\stretch{5}}
%\begin{flushright}
%\itshape A mio nonno Augusto per essere stato il 
%        \\primo a credere nelle mie capacità,
%        \\per avermi ispirato a non arrendermi mai
%        \\e a seguire sempre la mia strada
%\end{flushright}
%\vspace{\stretch{2}}
%\cleardoublepage

%\begin{minipage}[t]
%  {0.90\textwidth}
%  \raggedleft
%	{{\it  }}
%\end{minipage}

\tableofcontents
\thispagestyle{empty}
\listoffigures

\thispagestyle{empty}
\clearpage
\setcounter{page}{1}
\addtocontents{toc}{\protect\thispagestyle{empty}}

\input{Introbuzione.tex}

\clearpage
% contesto, obiettivo del lavoro, organizzazione tesi
\input{Contesto.tex}
% software define networking 
%intent-based networking
%controller stato dell'arte

\input{Teraflow.tex}
%motivi nascita teraflow
%sintesi architettura
%componenti service e policy
%P4
%gRPC
%Mininet
\input{Policy.tex}
\input{Conclusioni.tex}

\input{Appendice.tex}

%\bibliographystyle{plain} % We choose the "plain" reference style
%\bibliography{bibliography}
%per controller sdn https://dl.acm.org/doi/epdf/10.1145/2534169.2491193

\bibliographystyle{unsrt} 
\bibliography{bibliografia}
\input{Ring.tex}

\end{document}